...
JOIN answers a ON i.intent_id = a.intent_id;
```
FULL LỆNH MẪU BỔ SUNG CHO DATABASE
```bash
USE elevator_ai;
...
mysql -u elevator_ai -p elevator_ai < database/seed_full.sql
```
Kiểm tra xem đã thêm dữ liệu chưa:
```bash
mysql -u elevator_ai -p elevator_ai
```
Trong mysql:
```bash
SELECT COUNT(*) AS employees FROM employees;
SELECT COUNT(*) AS intents FROM intents;
SELECT COUNT(*) AS prompts FROM prompts;
SELECT COUNT(*) AS answers FROM answers;

SELECT i.intent_name, COUNT(p.prompt_id) AS prompt_count
FROM intents i
LEFT JOIN prompts p ON i.intent_id = p.intent_id
GROUP BY i.intent_name
ORDER BY prompt_count DESC;

EXIT;
```
Lệnh build lại embedding:
```bash
